...
class Connect4JFrame extends JFrame implements ActionListener {
        ...
        private static final int SIMPLE_MINIMAX = 3;
        private MenuItem simpleMiniMaxRedMI, simpleMiniMaxYellowMI;
        ...
        public Connect4JFrame() {
                ...
                simpleMiniMaxRedMI = new MenuItem("Simple Minimax Red");
                simpleMiniMaxRedMI.addActionListener(this);
                playersMenu.add(simpleMiniMaxRedMI);
                simpleMiniMaxYellowMI = new MenuItem("Simple Minimax Yellow");
                simpleMiniMaxYellowMI.addActionListener(this);
                playersMenu.add(simpleMiniMaxYellowMI);
                ...
                if (!end && isGameRunning) {
                        ...
                        if (currentTurn == RED) {
                                ...
                                if (yellowPlayerType == RANDOM) {
                                        ...
                                        putDisk(col + 1);
                                } else if (yellowPlayerType == SIMPLE_MINIMAX) {
                                        int col = SimpleMinimax.getBestMove(theArray, YELLOW);
                                        putDisk(col + 1);
                                }
                        }
        ...
        public void actionPerformed(ActionEvent e) {
                ...
                } else if (e.getSource() == simpleMiniMaxRedMI) {
                        redPlayerType = SIMPLE_MINIMAX;
                        if (currentTurn == RED && isGameRunning) {
                                makeMove();
                        }
                } else if (e.getSource() == simpleMiniMaxYellowMI) {
                        yellowPlayerType = SIMPLE_MINIMAX;
                        if (currentTurn == YELLOW && isGameRunning) {
                                makeMove();
                        }
                }
        } // end ActionPerformed
...
class MinimaxConnect4Player {
        ...
        private static boolean checkLine(int[][] board, int startRow, int startCol, int player, int count, int dRow,
                        int dCol) {
                int playerCount = 0;
                int emptyCount = 0;
                for (int i = 0; i < 4; i++) {
...
class SimpleMinimax {
        ...
        public static int getBestMove(int[][] board, int player) {
                ...
                for (int col = 0; col < Connect4JFrame.MAXCOL; col++) {
                        if (isValidMove(board, col)) {
                                int[][] newBoard = makeMove(board, col, player);
                                int score = minimax(newBoard, MAX_DEPTH, opponent);
                ...
                long endTime = System.currentTimeMillis();
                long duration = endTime - startTime;
                System.out.println("Simple Minimax move time: " + duration + "ms");
        ...
        private static int minimax(int[][] board, int depth, int player) {
                if (depth == 0 || isGameOver(board)) {
                        return evaluateBoard(board, player);
                }
